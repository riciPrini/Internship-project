...
def parser(depart,lastAct):
    if(depart is None):
        #print(lastAct.start_time)
        depart="0"
    elif(type(depart) is str):
        ftr = [3600,60,1]
        depart=str(sum([a*b for a,b in zip(ftr, map(int,depart.split(':')))]))+".00"
    return depart

def writeLeg(outf, lastAct, options, idveh, leg, startLink, endLink):
    #print(lastAct)
    depart = parser(leg.dep_time,lastAct)  if options.vehicles_only else "triggered"
    ...
    for person in sumolib.xml.parse(options.plan_file, 'person'):
        ...
        for item in plan.getChildList():
            ...
            if "act" in item.name:  # act or activity
                if lastLeg is not None and lastAct.type =="w":
                    # print(lastAct.start_time,item.link,lastAct.link,idveh)
                    # print(lastAct.end_time,item.link,lastAct.link,idveh)
                    leg = lastLeg
                    #departure
                    leg.dep_time = lastAct.end_time
                    writeLeg(outf, lastAct, options, idveh, leg, lastAct.link, item.link)
                    #arrival
                    leg.dep_time = lastAct.start_time
                    writeLeg(outf, lastAct, options, idveh, leg, item.link, lastAct.link)
                    lastLeg = None
                lastAct = item
            if item.name == "leg":
                if item.route is None:
                    ...
                    leg = item
                    writeLeg(outf, lastAct, options, idveh, leg, leg.route[0].start_link, leg.route[0].end_link)
            if leg:
